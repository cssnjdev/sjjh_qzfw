...
/**
 * <p>
 * File: FileExConfService.java
 * </p>
 * <p>
 * Title: FileExConfService
 * </p>
 * <p>
 * Description: 数据交换前置服务根据excel文件配置生成业务表的逻辑service
 * </p>
 *
 * @author cssnj
 * @version 1.0
...
public class FileExConfService {
	...
	/**
	 * <p>
	 * 方法名：generate_metadata_table
	 * </p>
	 * <p>
	 * Description:数据交换前置服务根据excel文件配置生成业务表的逻辑处理
	 * </p>
	 *
	 * @version 1.0
	 */
	...
	public ResponseEvent generate_metadata_table(RequestEvent requestEvent) throws Exception {
		...
		if (dataSourceMap.get(dataSource_biz_id) != null) {
			ConcurrentHashMap<String, String> db_info_biz_Map = (ConcurrentHashMap) dataSourceMap
					.get(dataSource_biz_id);
		...
		TransactionStatus status_biz = null;
		DataSourceTransactionManager transactionManager_biz = null;
		...
		try {
			...
			params.add(mb_id);
			List<Map> result = dataSourceUtils_conf
					.queryforlist(SystemContext.getSql("SQL_JHDL_GET_FILE_CONFIG_EXCEL_SHEET"), params);
			if (result != null && result.size() > 0) {
				Map map_tab = null;
				String table_name = null, table_ms = null, table_schema = null, sql_tab_ms_tmp = null,
						sql_col_ms_create = null, sql_col_edit = null, sheet_index = null;
				...
				for (int i = 0; i < result.size(); i++) {
					...
					if (table_name != null && !"".equals(table_name) && table_schema != null
							&& !"".equals(table_schema)) {
						try {
							colList = null;
							tabCommList = null;
							...
							if (db_type != null && "MYSQL".equals(db_type)) {
								if (table_schema != null && !"".equals(table_schema)) {
									...
									colList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_STRUCTURE_BY_TBNAME_OWNER_MYSQL"),
											params_tab_mx);
									tabCommList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_COMMENTS_BY_TBNAME_MYSQL"),
											params_tab_mx);
								} else {
									params_tab_mx.add(table_name);
									colList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_STRUCTURE_BY_TBNAME_MYSQL"),
											params_tab_mx);
									tabCommList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_COMMENTS_BY_TBNAME_MYSQL"),
											params_tab_mx);
								...
								if (table_schema != null && !"".equals(table_schema)) {
									...
									colList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_STRUCTURE_BY_TBNAME_OWNER_ORACLE"),
											params_tab_mx);
									tabCommList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_COMMENTS_BY_TBNAME_ORACLE"),
											params_tab_mx);
								} else {
									params_tab_mx.add(table_name.toUpperCase());
									colList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_STRUCTURE_BY_TBNAME_ORACLE"),
											params_tab_mx);
									tabCommList = dataSourceUtils_biz.queryforlist(
											SystemContext.getSql("SQL_JHDL_GET_TABLE_COMMENTS_BY_TBNAME_ORACLE"),
											params_tab_mx);
							...
							if (tabCommList != null && tabCommList.size() > 0) {
								...
								while (iter_tab_comms.hasNext()) {
									...
									if (db_tabcomms_map.get("TABLE_NAME") != null
											&& !"".equals(db_tabcomms_map.get("TABLE_NAME"))) {
										if (!db_tabcomms_map.get("TABLE_COMMENT").equals(table_ms)) {
											sql_col_edit = "comment on table " + table_schema + "." + table_name
													+ " is '" + table_ms + "'";
							...
							String conf_column_name = null, conf_column_ms = null, conf_data_type = null,
									conf_data_length = null;
							params_mx = new ArrayList();
							params_mx.add(mb_id);
							params_mx.add(table_name);
							params_mx.add(sheet_index);
							result_conf = dataSourceUtils_conf.queryforlist(
									SystemContext.getSql("SQL_JHDL_GET_FILE_CONFIG_EXCEL_COLMX"), params_mx);
							...
							if (colList != null && colList.size() > 0) {
								...
								String db_column_name = null, db_data_type = null, db_data_length = null,
										db_column_ms = null;
								...
								if (iter_conf.hasNext()) {
									while (iter_conf.hasNext()) {
										...
										dbConfColNameColList
												.add(((String) db_conf_resmap.get("DB_COLMC")).toUpperCase());
									}
								}
								...
								while (iter_db.hasNext()) {
									...
									db_data_length = db_resmap.get("data_length") == null ? ""
											: db_resmap.get("data_length").toString();
									db_column_ms = db_resmap.get("comments") == null ? ""
											: db_resmap.get("comments").toString();
									if (result_conf != null && result_conf.size() > 0) {
										...
										if (!dbConfColNameColList.contains(db_column_name)) {
											sql_col_edit = "alter table " + table_schema + "." + table_name
													+ " drop column " + db_column_name.toLowerCase();
											st.addBatch(sql_col_edit);
											...
											while (iter_conf.hasNext()) {
												...
												if (db_column_name.toUpperCase().equals(conf_column_name.toUpperCase())
														&& db_data_type.toUpperCase()
																.equals(conf_data_type.toUpperCase())
														&& db_data_length.equals(conf_data_length)
														&& db_column_ms.equals(conf_column_ms)) {
													continue;
												} else {// 有配置修改的地方
														// 表字段存在但其余属性发生变化
													if (db_column_name.toUpperCase()
															.equals(conf_column_name.toUpperCase())) {
														// 修改了字段类型或字段长度
														if (!db_data_type.toUpperCase()
																.equals(conf_data_type.toUpperCase())
																|| !db_data_length.equals(conf_data_length)) {
															sbf_sqledit = new StringBuffer("alter table ")
																	.append(table_schema).append(".")
																	.append(table_name.toUpperCase()).append(" modify ")
																	.append(conf_column_name.toLowerCase());
															sbf_sqledit = getDbCreatSql(conf_data_type,
																	conf_data_length, sbf_sqledit);
															st.addBatch(sbf_sqledit.toString());
														}
														// 修改了字段描述
														if (!db_column_ms.equals(conf_column_ms)) {
															sql_col_edit = "comment on column " + table_schema + "."
																	+ table_name + "." + conf_column_name.toLowerCase()
																	+ " is '" + conf_column_ms + "'";
															st.addBatch(sql_col_edit);
														}
													} else {// 字段不匹配的情况
														...
														if (dbColNameColList.contains(conf_column_name.toUpperCase())) {
															...
															if (conf_column_name != null && !"".equals(conf_column_name)
																	&& conf_data_type != null
																	&& !"".equals(conf_data_type)) {
																sbf_sqledit = new StringBuffer("alter table ")
																		.append(table_schema).append(".")
																...
																sbf_sqledit = getDbCreatSql(conf_data_type,
																		conf_data_length, sbf_sqledit);
																st.addBatch(sbf_sqledit.toString());
																if (conf_column_ms != null
																		&& !"".equals(conf_column_ms)) {
																	sql_col_edit = "comment on column " + table_schema
																			+ "." + table_name + "."
																			+ conf_column_name.toLowerCase() + " is '"
																			+ conf_column_ms + "'";
																	st.addBatch(sql_col_edit);
																}
								...
								sbf_sqlcreate = new StringBuffer(
										"create table " + table_schema + "." + table_name + " (");
								sql_tab_ms_tmp = "comment on table " + table_schema + "." + table_name + " is '"
										+ table_ms + "'";
								if (result_conf != null && result_conf.size() > 0) {
									// st = conn_biz.createStatement();
									ms_list = new ArrayList();
									while (iter_conf.hasNext()) {
										...
										conf_data_type = (String) db_conf_resmap.get("db_colmc_type");
										conf_data_length = db_conf_resmap.get("db_colmc_len") + "";
										sql_col_ms_create = "comment on column " + table_schema + "." + table_name + "."
												+ conf_column_name.toLowerCase() + " is '" + conf_column_ms + "'";
...
//                                }
			...
			resEvent.setResStr(BizErrorMsgContext.singleton().getValueAsString("BIZ.SJJHPT.QZFW.WBWSFW.ERR_00_0009")
					+ "," + e.toString());
			resEvent.setResCode(-1);
			return resEvent;
		} finally {
...
//            transactionManager_biz.commit(status_biz);
	...
	private StringBuffer getDbCreatSql(String conf_data_type, String conf_data_length, StringBuffer sbf_sqledit) {
		...
		} else if ("TIMESTAMP".equals(conf_data_type.toUpperCase())
				|| "DATETIME".equals(conf_data_type.toUpperCase())) {
			sbf_sqledit.append(" TIMESTAMP ");
		} else if ("NUMBER".equals(conf_data_type.toUpperCase()) || "INT".equals(conf_data_type.toUpperCase())
				|| "BIGINT".equals(conf_data_type.toUpperCase())) {
