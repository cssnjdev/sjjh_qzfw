...
import com.cwks.bizcore.sjjh.core.vo.ExcelVersion;
...
//            wb = WorkbookFactory.create(new FileInputStream(file));
        ...
        ExcelSheetPO sheetPO=null;
        int readRowCount,readColumnCount;
        Row row =null;
        List<Object> rowValue=null;
        for (int i = 0; i < wb.getNumberOfSheets(); i++) {
            sheet = wb.getSheetAt(i);
            dataList = new ArrayList<>();
            sheetPO = new ExcelSheetPO();
            ...
            for (int j = sheet.getFirstRowNum(); j < readRowCount; j++) {
               row = sheet.getRow(j);
                ...
                rowValue = new LinkedList<Object>();
                // 解析sheet 的列
    ...
    private static Object getCellValue(Workbook wb, Cell cell) {
        ...
        if (cell != null) {
            //df = new DecimalFormat("0");// 格式化 number
            // String
            // 字符
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");// 格式化日期字符串
            //snf = new DecimalFormat("0.00");// 格式化数字
            ...
            case Cell.CELL_TYPE_NUMERIC:
            	if (HSSFDateUtil.isCellDateFormatted(cell)) {  //判断日期类型
                	...
                	columnValue = new DecimalFormat("#.######").format(cell.getNumericCellValue()); 
                } 
