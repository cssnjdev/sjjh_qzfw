...
public class JhQzFwContext{
	...
	//mq
	private static String JHDL_CORE_NBFW_MQ_CONFIG_ISOPEN = BizContext.singleton().getValueAsString("biz.sjjhpt.qzfw.core.nbfw.mq.conf.isopen");

	private static String JHDL_CORE_NBFW_MQ_CONFIG_TLD = BizContext.singleton().getValueAsString("biz.sjjhpt.qzfw.core.nbfw.mq.conf.tld");
	...
	public JhQzFwContext() {
        try {
			...
			LogWritter.sysInfo("初始化系统异常："+e.getMessage());
		}
    }
	...
	public void init() throws ServletException {
		LogWritter.sysInfo("===============JhQzFwContext starting Initialize (jhdl)==================>>>>>");
		if (this.initialized) {
			return;
		}
		//如果启用非中心数据交换模式，从本地配置文件加载资源规则和数据源信息
		if(ComProConf.JHDL_RESTAPI_NBFW_URL_ISLOCAL != null && "true".equals(ComProConf.JHDL_RESTAPI_NBFW_URL_ISLOCAL)){
			// 加载wsrules.xml配置信息-----
			startResourceDealRulesByLocal();
			startDataSourcesByLocal();
			startResourceClientsByLocal();
		}else{//从交换管理中心获取配置规则
			startResourceAndDataSourcesInfoByCenter();
		}
		setHost();
		this.setInitialized(true);
		LogWritter.sysInfo("===============JhQzFwContext end Initialized (jhdl)==================!!!!!");
	}
	...
	private void setHost(){
		try {
			...
			LogWritter.sysInfo("Host not found: " + e.getMessage());
		}
	}

	private void startResourceAndDataSourcesInfoByCenter() {
		LogWritter.sysInfo("--------------- 开始从交换管理中心加载前置服务资源处理规则和数据源信息------------->");
		try {
			String result=null;
			JSONObject obj=null;
				...
				if (response.getCode() == 200) {
					...
					if(obj != null){
						...
						if ((resourcesMap == null || resourcesMap.size()<=0) &&(datasourcesMap == null || datasourcesMap.size()<=0)) {
							LogWritter.sysError("ERROR:交换管理中心推送的配置信息[接入点前置资源信息和数据源信息]为空",null);
							System.exit(0);
						}else{
			...
			LogWritter.sysError("从交换管理中心加载前置服务资源处理规则和数据源信息错误：",e);
		}
		LogWritter.sysInfo("--------------- 加载交换管理中心前置服务资源处理规则和数据源信息结束------------->");
	}


	private void startResourceDealRulesByLocal() {
		LogWritter.sysInfo("--------------- 开始加载前置服务资源处理规则------------->");
		...
		try {
			...
			try {
				for (int i = 0; i < list.size(); i++) {
					...
					LogWritter.sysInfo("加载资源id：" + id + "(" + des + ")交换服务配置规则.....");
				...
				LogWritter.sysInfo("加载资源id：\" + id + \"(\" + des + \")交换服务配置规则出现错误!");
			}
		} catch (Exception e) {
			LogWritter.sysInfo("加载resources/config/jhdl/jhqzfw_resources.xml出现错误："+e.getMessage());
		}
		LogWritter.sysInfo("--------------- 加载前置服务资源处理规则结束------------->");
	}
	private void startDataSourcesByLocal() {
		LogWritter.sysInfo("--------------- 开始加载前置服务资源处理规则------------->");
		SAXBuilder builder = new SAXBuilder();
		try {
			Document doc = builder.build(this.getClass().getResource("/config/jhdl/datasources.xml"));
			Element root = doc.getRootElement();
			List<?> list = root.getChildren();
			Element ruleElm = null;
			String id, des,type;
			...
			LogWritter.sysInfo("加载resources/config/jhdl/datasources.xml出现错误："+e.getMessage());
		}
		LogWritter.sysInfo("--------------- 加载前置服务资源处理规则结束------------->");
	}
	private void startResourceClientsByLocal() {
		LogWritter.sysInfo("--------------- 开始加载前置服务资源处理规则------------->");
		SAXBuilder builder = new SAXBuilder();
		try {
			Document doc = builder.build(this.getClass().getResource("/config/jhdl/jhqzfw_resources_clients.xml"));
			Element root = doc.getRootElement();
			List<?> list = root.getChildren();
			Element ruleElm = null;
			String id, des,reqResourceId = "";
			ConcurrentHashMap<String, String> reqResourceMap = null;
			ConcurrentHashMap<String, Object> tempMap = null;
			Map<String,Object> reqResources=null;
			resourceClientsMap = new ConcurrentHashMap<String, ConcurrentHashMap<String, Object>>();
			List<Element> listE=null;
			List<Element> l=null,ll=null;
			try {
				for (int i = 0; i < list.size(); i++) {
					ruleElm = (Element) list.get(i);
					id = ruleElm.getAttributeValue("id");
					...
					tempMap.put("clientId", id);
					if (des == null) {des = "";}
					tempMap.put("des", des);
					listE=ruleElm.getChildren();
					reqResources=new HashMap<String,Object>();
					for(Element e:listE) {
						l=e.getChildren();
						if(l==null||l.size()==0) {
							tempMap.put(e.getName(), e.getText());
							...
							for(Element ee:l) {
								reqResourceMap=new ConcurrentHashMap<String, String>();
								ll=ee.getChildren();
								for(Element eee:ll) {
									if(eee.getName().equals("reqResourceId")) {
										reqResourceId=eee.getText();
									}
									reqResourceMap.put(eee.getName(), eee.getText());
								}
								reqResources.put(reqResourceId,reqResourceMap);
							}
							tempMap.put(e.getName(), reqResources);
						}
					}
					resourceClientsMap.put(id, tempMap);
				}
			} catch (Exception e) {
				LogWritter.sysInfo("加载资源id：\" + id + \"(\" + des + \")交换服务配置规则出现错误!");
			}
	...
	public String getJhdlCoreNbfwMqConfigIsopen() {
		return JHDL_CORE_NBFW_MQ_CONFIG_ISOPEN;
	}

	public String getJhdlCoreNbfwMqConfigTld() {
		return JHDL_CORE_NBFW_MQ_CONFIG_TLD;
	}
