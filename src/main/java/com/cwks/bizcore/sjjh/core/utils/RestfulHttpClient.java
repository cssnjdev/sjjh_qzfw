...
//            }
    ...
    private static HttpResponse request(HttpClient client) throws IOException {
    	LogWritter.bizDebug("进入RestfulHttpClient_request:::::::client:::::::::"+client.getUrl());
        HttpURLConnection con = instance(client);
        if (METHOD_GET.equalsIgnoreCase(client.getMethod())) {
            //GET请求，不用发请求体
        	LogWritter.bizDebug("进入RestfulHttpClient_request:::::::Get");
            return readResponse(con);
        }
        LogWritter.bizDebug("进入RestfulHttpClient_request:::::::POST");
        ...
        bw.close();
        LogWritter.bizDebug("进入RestfulHttpClient_request:::::::con::::::::"+con.toString());
        return readResponse(con);
    }
    ...
    private static HttpResponse readResponse(HttpURLConnection con) throws IOException {
        try {
        	LogWritter.bizDebug("\n进入RestfulHttpClient——readResponse开始:::::::con.getInputStream()");
            BufferedReader br = new BufferedReader(new InputStreamReader(con.getInputStream(), charset));
            LogWritter.bizDebug("\n进入RestfulHttpClient——readResponse结束:::::::con.getInputStream()");
            ...
            br.close();
            LogWritter.bizDebug("\n进入RestfulHttpClient__sb.toString():::::::"+sb.toString());
